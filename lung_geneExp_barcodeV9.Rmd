...
The question is: Why does a subset of NS-Forest genes or genes that are differentially expressed have a low expression probabilities in their target cell type? The hypothesis is that since NS-Forest genes are only expressed in one or a few cell types and often expressed in low level, the model might be challenged to classify their expression state and expression probility.

2.2 The rationales behind each analysis step
1.Make distribution plots for the markers with wrong expProb.
Studing the expression distribution of the subset gene in all cell types and their target cell type may shed a light about the reason that wrong classification and wrong probability.

2.Check all prior and post parameter measure for the markers with wrong expProb, especially lamda.
Studing all prior and post parameter measure for the markers with wrong expProb, especially lamda may help us to improve the model
...
generateCellType_barcode2 <- function(seurat_dataset,dataset_name, cellType, cut_off){
  # subset by cell type
  if(dataset_name=="hlca") {
  data_subset <- subset(seurat_dataset, subset = ann_finest_level == cellType)
  } else if(dataset_name=="cellRef"){
...
runBatch2<-function(seurat_dataset,dataset_name,cell_type_list,cut_off_list){
  dic ={}
  for(cellType in cell_type_list){
   
    for (cut_off in cut_off_list) {
      
      code<-generateCellType_barcode2(seurat_dataset, dataset_name,cellType, cut_off)
      print(head(code))
      dic[[cellType]]<-as.data.frame(code) 
    }
  }
...
barcode_dic2<-runBatch2(seurat_dataset,dataset_name,cell_type_list,cut_off_list)
saveRDS(barcode_dic2, file = "hlca_barcode_detail.rds")

barcode_dic2 <- readRDS("hlca_barcode_detail.rds")
...
write.table(marker_hlca_prior_post, 'marker_hlca_prior_post_update.tsv', sep = "\t",row.names = FALSE,col.names = TRUE) 
...
on_hlca_post <- read.table("marker_hlca_prior_post_update.tsv", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
