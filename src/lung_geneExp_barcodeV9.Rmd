...
The question is: why does a subset of NS-Forest genes or differentially expressed genes have low expression probabilities in their target cell type? The hypothesis is that the parameters were trained on data collected before 2019. After re-training, the performance could be improved.


## 2.2 The rationales behind each analysis step

### 2.2.1 Calculate expression probability (expProb) using the updated parameter 'params_update_hlca.rda' from re-training

The old parameters (params_EM_81020.rda) were trained on data collected before 2019. After re-training, the performance could be improved.

### 2.2.2 Calculate recall and adjust precision when using NS-forest gene as true positive for validation

To investigate whether re-training improves recall without worsening precision.

### 2.2.3 Analyze all prior and post measurements for the NS-forest markers 
...
## 7. Validate the expression probabilities 

### 7.1 calculate rate and three probability for marker genes
...
write.table(marker_hlca_prior_post_rate, 'marker_hlca_prior_post_detail.tsv', sep = "\t",row.names = FALSE,col.names = TRUE) 


```


### 7.2 calculate update expProb for marker genes
...
### 7.3. Check the "on" state probability of NS-forest markers and calculate the confusion matrix. 
...
#### 7.3.1 Probabilty cut off 0.75

```{r, message=FALSE,warning=FALSE}
on_detail_post <- read.table("marker_hlca_prior_post_detail.tsv", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

for cutoff in c(0.75, 0.90,0.95)
on_hlca_post<-on_detail_post %>% mutate(expected_value= ifelse(targeted=="TRUE", 1,0 ),
                                            predicted_value= ifelse(ln2 >0.75, 1,0 ))
...
on_matrix <- confusionMatrix(data=on_hlca_post$predicted_value, reference = on_hlca_post$expected_value,positive = "1")

on_matrix

```

#### 7.3.2 Probabilty cut off 0.90

```{r, message=FALSE,warning=FALSE}
on_hlca_post<-on_detail_post %>% mutate(expected_value= ifelse(targeted=="TRUE", 1,0 ),
                                            predicted_value= ifelse(ln2 >0.9, 1,0 ))
...
#### 7.3.3 Probabilty cut off 0.95

```{r, message=FALSE,warning=FALSE}
on_hlca_post<-on_detail_post %>% mutate(expected_value= ifelse(targeted=="TRUE", 1,0 ),
                                            predicted_value= ifelse(ln2 >0.95, 1,0 ))
