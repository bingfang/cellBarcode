...
geneList_bi<-merge(bi_gene,gene_Ensemble, by.x = "symbol", by.y = "gene_name", all.x = TRUE) 
...
prior_marker<-merge(geneList, pi_df, by = "ENSEMBLID", all.x = TRUE) 
...
prior_bi<-merge(geneList_bi, pi_df, by = "ENSEMBLID", all.x = TRUE) 
prior_bi$prior_prob_off <- prior_bi$V1 + prior_bi$V2
prior_bi$prior_prob_on <- 1-(prior_bi$V1 + prior_bi$V2)

hist(as.numeric(prior_bi$prior_prob_on),main = "", xlab = "Prior probability of expression of the Binary genes")
...
prior_marker$marker <-paste(prior_marker$cluster,prior_marker$dataset,sep="_")
df_hlca_gene<-checkGene(prior_marker$ENSEMBLID,prior_marker$symbol, barcode_dic=barcode_dic2, prior_marker$marker, data_name)
df_hlca_gene<-df_hlca_gene %>% mutate ()
